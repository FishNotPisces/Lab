...
public class Text implements Container{
    ...
    public void sort()
    {
        mergeSort(this.wordlist, this.wordcount);
    }

    private void mergeSort(String[] list, int listLength)
    {
        if (listLength < 2) return;
        String[] left = new String[(int)(listLength/2)];
        System.arraycopy(list, 0, left, 0, left.length);
        String[] right = new String[(int)(listLength/2 + 0.5)];
    ...
    {
        int i[] = new int[2];
        while (i[0] < left.length || i[1] < left.length)
        {
